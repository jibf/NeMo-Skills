...
STATIONS_DB = {
    ...
    (35.6762, 139.6503): [{"station_id": "RJTT", "name": "Tokyo Haneda Airport", "distance": 15.0}],
}
...
def get_hourly_observation(station_id: str, start_time: str, end_time: str, time_zone: str) -> List[str]:
    ...
    for single_date in (start_date + timedelta(n) for n in range((end_date - start_date).days + 1)):
        day_of_week = single_date.strftime("%A")  # Get the day of the week (e.g., "Monday")
        daily_observations = HOURLY_OBSERVATIONS.get(station_id, {}).get(day_of_week, [])
