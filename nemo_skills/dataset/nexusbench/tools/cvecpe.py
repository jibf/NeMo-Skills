...
CPE_DB = {
    ...
    "Windows": [
        ...
        },
        {
            ...
            "refs": [{"ref": "https://www.microsoft.com/windows-server", "type": "Vendor"}],
        },
        {
    ...
    "Windows 10": [
        {
            ...
            "refs": [{"ref": "https://support.microsoft.com/en-us/windows", "type": "Vendor"}],
        },
    ],
    "Windows 8": [
        {
    ...
    "Java": [
        ...
        },
        {
            ...
            "titles": [{"title": "Oracle Java Runtime Environment 16.0.0", "lang": "en"}],
            "refs": [{"ref": "https://www.oracle.com/java/", "type": "Vendor"}],
        },
        {
        ...
        },
        {
            ...
            "titles": [{"title": "Oracle Java Runtime Environment 11.0.10", "lang": "en"}],
            "refs": [{"ref": "https://www.oracle.com/java/", "type": "Vendor"}],
        },
        {
        ...
        },
        {
            ...
            "refs": [{"ref": "https://spring.io/projects/spring-framework", "type": "Vendor"}],
        },
    ],
    "Zephyr": [
        {
...
def searchCPE(
        ...
        for cpe in cpes:
            if all(
                [
                    ...
                    (not cpeMatchString or cpeMatchString in cpe["cpeName"]),  # Changed to partial match
                    (
                        not keywordSearch
                        or (keywordExactMatch and keywordSearch in [title["title"] for title in cpe["titles"]])
                        or (
                            not keywordExactMatch
                            and any(keywordSearch.lower() in title["title"].lower() for title in cpe["titles"])
                        )
                    ),
                    (not matchCriteriaId or matchCriteriaId in cpe.get("matchCriteria", [])),
                ]
            ):
                results.append(cpe)
    ...
    if lastModStartDate and lastModEndDate:
        ...
        results = [cpe for cpe in results if start <= datetime.strptime(cpe["lastModified"][:10], "%Y-%m-%d") <= end]
    ...
    **kwargs,
    ...
    for cves in CVE_DB.values():
        for cve in cves:
            if all(
                [
                    (not keywordSearch or keywordSearch.lower() in cve["description"].lower()),
                    (not cvssV2Severity or cve.get("cvssV2Severity", "").lower() == cvssV2Severity.lower()),
                    (not cvssV3Severity or cve.get("cvssV3Severity", "").lower() == cvssV3Severity.lower()),
                    (not cvssV2Metrics or cve.get("cvssV2Metrics") == cvssV2Metrics),
                    (hasCertAlerts is None or cve.get("hasCertAlerts") == hasCertAlerts),
    ...
    if lastModStartDate and lastModEndDate:
        ...
        results = [
            cve for cve in results if start <= datetime.strptime(cve["lastModifiedDate"][:10], "%Y-%m-%d") <= end
        ]

    if pubStartDate and pubEndDate:
        start = datetime.strptime(pubStartDate, "%Y-%m-%d")
        end = datetime.strptime(pubEndDate, "%Y-%m-%d")
        results = [cve for cve in results if start <= datetime.strptime(cve["publishedDate"][:10], "%Y-%m-%d") <= end]
...
def summarize_cvecpes(cvecpeList: List[Dict[str, Any]]) -> str:
    ...
    for item in cvecpeList:
        ...
        if summary["last_modified_range"]["earliest"] is None or mod_date < summary["last_modified_range"]["earliest"]:
            summary["last_modified_range"]["earliest"] = mod_date
        if summary["last_modified_range"]["latest"] is None or mod_date > summary["last_modified_range"]["latest"]:
...
"""
    ...
    if summary["product_types"]:
        ...
        formatted_summary += "\n".join([f"  - {k}: {v}" for k, v in summary["product_types"].items()])
        formatted_summary += "\n"
...
def search_backup_keywords(cvecpeList: List[Dict[str, Any]], backup_keyword: str) -> List[Dict[str, Any]]:
    """
...
def compare_cvecpes(cvecpeList1: List[Dict[str, Any]], cvecpeList2: List[Dict[str, Any]]) -> str:
    """compare_cvecpes"""
    set1 = set(item["cveId"] if "cveId" in item else item["cpeName"] for item in cvecpeList1)
    set2 = set(item["cveId"] if "cveId" in item else item["cpeName"] for item in cvecpeList2)
...
def mergeCVEs(list1: List[Dict[str, Any]], list2: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def mergeCPEs(list1: List[Dict[str, Any]], list2: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def countCVEsBySeverity(cve_list: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    """
    severity_counts = Counter(cve["cvssV3Severity"] for cve in cve_list if "cvssV3Severity" in cve)
    return {
...
def sortCVEsByCVSSv3Score(cve_list: list, descending: bool = True) -> list:
    ...
    """
    return sorted(cve_list, key=lambda cve: cve.get("cvssV3Score", 0), reverse=descending)
...
def sortCVEsByCVSSv2Score(cve_list: list, descending: bool = True) -> list:
    ...
    list: Sorted list of CVE objects.
    """
    return sorted(cve_list, key=lambda cve: cve.get("cvssV2Score", 0), reverse=descending)
...
def sortCVEsByModDate(cve_list: list, descending: bool = True) -> list:
    ...
    return sorted(
        cve_list,
        key=lambda cve: datetime.strptime(cve["lastModifiedDate"][:19], "%Y-%m-%dT%H:%M:%S"),
        reverse=descending,
    )
...
def sortCPEsByLastMod(cpeList: list, descending: bool = True) -> list:
    ...
    return sorted(
        cpeList,
        key=lambda cpe: datetime.strptime(cpe["lastModified"][:19], "%Y-%m-%dT%H:%M:%S"),
        reverse=descending,
    )
...
def filterCVEsBySeverity(cveList: List[Dict[str, Any]], severityLevel: str) -> List[Dict[str, Any]]:
    """
    ...
    return [
        ...
        or (cve.get("cvssV2Severity", "").upper() == severityLevel and severityLevel != "CRITICAL")
    ]


def filterCVEByLanguage(cve_list: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
    """
    ...
    """
    return [cve for cve in cve_list if any(desc.get("lang") == language for desc in cve.get("descriptions", []))]
