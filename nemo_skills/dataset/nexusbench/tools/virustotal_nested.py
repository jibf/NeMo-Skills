...
mock_db = {
    "domains": {
        ...
        "stackoverflow.com": {"comments": ["A question-and-answer website for computer programmers"]},
    ...
    "ip_addresses": {
        ...
        "152.678.234.60": {"comments": ["Often flagged for suspicious activities", "High risk IP"]},
        "192.168.1.2": {
            "related_threat_actors": [],
        ...
        "196.84.239.213": {"comments": ["Periodic pings observed, possibly for keeping connections alive."]},
        ...
        "34.74.55.1": {
            "related_threat_actors": [{"name": "HackerX", "source_region": "North America"}],
...
def vt_get_comments_on_multiple_domains(domains, x_apikey, limit=None, cursor=None):
    ...
    for domain in domains:
        if domain in mock_db["domains"]:
            comments[domain] = (
                mock_db["domains"][domain]["comments"][:limit] if limit else mock_db["domains"][domain]["comments"]
            )
        else:
...
def vt_is_date_within_range(timestamp, start=None, end=None):
    ...
    start_date = datetime.datetime.strptime(start, "%Y/%m/%d") if start and len(start) == 10 else None
    end_date = datetime.datetime.strptime(end, "%Y/%m/%d") if end and len(end) == 10 else None
...
def vt_get_threat_actors_latest_modification_date(threat_actor_objects, x_apikey):
    ...
    if threat_actor_objects:
        return max(actor.get("last_modification_date", 0) for actor in threat_actor_objects)
    return 0
...
def vt_get_threat_actors_main_source_region(threat_actors, x_apikey):
    ...
    """
    if threat_actors:
        regions = [actor["source_region"] for actor in threat_actors if "source_region" in actor]
        if regions:
...
def vt_validate_historical_ssl_certificates(historical_ssl_certificates, x_apikey):
    ...
    """
    return any(
        cert["valid"] and datetime.datetime.strptime(cert["expiration_date"], "%Y-%m-%d") > datetime.datetime.now()
...
def vt_get_comments_on_ip_address(ip, x_apikey, limit=None, cursor=None):
    ...
    - cursor, string, optional, Continuation cursor
    """
    return mock_db["ip_addresses"][ip]["comments"][:limit] if limit else mock_db["ip_addresses"][ip]["comments"]
...
def vt_get_object_descriptors_related_to_ip_address(ip, relationship, x_apikey, limit=None, cursor=None):
    ...
    data = {
        "comments": [{"comment": comment} for comment in mock_db["ip_addresses"][ip].get("comments", [])],
        "communicating_files": [{"file_name": f"file{index}.exe"} for index in range(5)],
        "downloaded_files": [{"file_name": f"download{index}.zip"} for index in range(3)],
        "graphs": [{"graph_id": f"graph{index}"} for index in range(2)],
        "historical_ssl_certificates": [{"certificate_id": f"cert{index}"} for index in range(4)],
        "historical_whois": [{"record_id": f"whois{index}"} for index in range(2)],
        "related_comments": [{"comment": "Related comment " + str(i)} for i in range(3)],
        "related_references": [{"reference": f"ref{index}"} for index in range(2)],
        "related_threat_actors": [
            {"actor_name": actor["name"]} for actor in mock_db["ip_addresses"][ip].get("related_threat_actors", [])
        ],
        "referrer_files": [{"file_name": f"referrer_file{index}.exe"} for index in range(3)],
...
def vt_get_object_descriptors_related_to_domain(domain, relationship, x_apikey, limit=None, cursor=None):
    ...
    descriptors = {
        "caa_records": [{"record": rec} for rec in mock_db["domains"][domain].get("caa_records", [])],
        "cname_records": [{"record": "cname.example.com"}],  # Static example
        "comments": [{"comment": comment} for comment in mock_db["domains"][domain].get("comments", [])],
        "communicating_files": [
            {"file_name": file} for file in mock_db["domains"][domain].get("communicating_files", [])
        ],
        "downloaded_files": [{"file_name": f"download{index}.mp4"} for index in range(3)],  # Example
        "graphs": [{"graph_id": f"graph{index}"} for index in range(2)],  # Example
        "historical_ssl_certificates": [{"certificate_id": f"cert{index}"} for index in range(4)],  # Example
        "historical_whois": [{"record_id": f"whois{index}"} for index in range(2)],  # Example
        "immediate_parent": [{"parent": "com"}],  # Example
        "mx_records": [{"record": record} for record in mock_db["domains"][domain].get("mx_records", [])],
        "ns_records": [{"record": "ns1.nameserver.com"}],  # Static example
        "parent": [{"parent": "com"}],  # Example
        "referrer_files": [{"file_name": f"referrer{index}.pdf"} for index in range(2)],  # Example
        "related_comments": [{"comment": "Related to example.com"}],  # Static example
        "related_references": [{"reference": f"ref{index}"} for index in range(2)],  # Example
        "related_threat_actors": [
            {"actor_name": actor["name"]} for actor in mock_db["domains"][domain].get("related_threat_actors", [])
        ],
        "resolutions": [{"resolution_id": f"res{index}"} for index in range(2)],  # Example
        "soa_records": [{"record": "soa.example.com"}],  # Static example
        "siblings": [{"domain": "sibling.example.com"}],  # Static example
        "subdomains": [{"domain": sub} for sub in mock_db["domains"][domain].get("subdomains", [])],
...
def vt_get_comments_on_domain(domain, x_apikey, limit=None, cursor=None):
    ...
    - cursor, string, optional, Continuation cursor
    """
    return mock_db["domains"][domain]["comments"][:limit] if limit else mock_db["domains"][domain]["comments"]
...
vt_get_majority_vote_json = {
    ...
    "parameters": {
        "type": "object",
        "properties": {"votes": {"type": "object", "description": "Dictionary of votes"}},
        "required": ["votes"],
    },
}
